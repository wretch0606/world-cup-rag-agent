...
class MockAgentService:
    ...
    def query(self, request: dict, trace_id: str) -> dict:
        ...
        if any(kw in question for kw in ["你好", "hello", "hi", "帮助"]):
            ...
            base["answer"] = (
                "你好！我是世界杯知识问答助手，可以回答关于世界杯比赛、比分、球队和晋级关系的问题。"
            )
            base["timing"]["total_ms"] = 2
            return base

        # Branch 2: 2022 final exact fact
        if ("2022" in question or "卡塔尔" in question) and (
            "决赛" in question or "final" in question.lower()
        ):
            base["intent"] = "match_result_query"
            base["route"] = "structured_query"
            base["answer"] = (
                "2022年世界杯决赛，阿根廷与法国在90分钟内战成2:2，加时赛后为3:3；阿根廷在点球大战中以4:2获胜。"
            )
            try:
                        ...
                        "edges": [
                            {
                                ...
                                "result_type": match["result_type"],
                                "winner_team_id": match.get("winner_team", {}).get("team_id")
                                if match.get("winner_team")
                                else None,
                                "label": (
                                    f"{match['tournament_year']} "
                                    f"{match.get('stage_name', '')} "
                                    f"{match['score']['display']}"
                                ),
                            }
                        ],
                    }
