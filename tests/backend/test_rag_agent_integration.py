...
class FakeRetrievalGateway:
    def __init__(self, *, items=None, status=RAGStatus.ok, warnings=None, error=None):
        self.items = items or []
        self.status = status
...
class FakeGenerationClient:
    ...
    async def generate(self, request, retrieval):
        ...
        return RAGResult(
            ...
            generation_meta=GenerationMeta(model_name="test-model"),
            error=None,
        )
...
# ====================================================================
def _invoke_agent(question, filters=None, trace_id="test-trace", rag_service="NOT_SET"):
    import importlib
    ...
    request = {"question": question, "session_id": None, "filters": filters or {}}
...
def _mk_svc(answer="OK", facts=None, sources=None, status=RAGStatus.ok):
    ...
    gen = FakeGenerationClient(
        answer=answer, facts=facts or [], sources=sources or [], status=status
    )
...
def test_exact_score_no_rag_calls():
    ...
    result = _invoke_agent("2022年世界杯决赛比分是多少？", rag_service=rag_svc)
    assert result["route"] == "structured_query"
...
def test_semantic_calls_rag_service():
    rag_svc, ret, gen = _mk_svc(
        answer="经典比赛分析。", sources=[SourceItem(source_id="s1", title="T")]
    )
...
def test_trace_id_passthrough():
    rag_svc, ret, gen = _mk_svc()
    _invoke_agent("介绍2022年世界杯决赛的重要情节", trace_id="my-custom-trace", rag_service=rag_svc)
    assert ret.last_request is not None
...
def test_rag_status_ok_mapping():
    ...
    result = _invoke_agent("介绍2022年世界杯决赛的重要情节", rag_service=rag_svc)
...
def test_rag_status_empty_mapping():
    ...
    gen = FakeGenerationClient(
        ...
        warnings=[WarningItem(code="NO_RESULT", message="x", component="retrieval")],
    )
...
def test_rag_error_mapping():
    ...
    ret = FakeRetrievalGateway(
        status=RAGStatus.error,
        error=ErrorItem(code="RETRIEVAL_ERROR", message="f", component="retrieval"),
    )
...
def test_mock_mode_still_works():
    ...
    result = svc.query(
        {"question": "2022年世界杯决赛比分是多少？", "session_id": None, "filters": {}}, "trace-x"
    )
